...
impl InputEditor {
    ...
    /// Enables the editor using `question`.
    pub fn enable(&mut self, question: &dyn Question) {
        self.prompt = Some(question.prompt());
        self.completer = question.completer();
        self.set_sizes();
        self.set_input(question.value());
