...
The _controller_ coordinates this interaction by recognizing that an operation bound to a key sequence returns a _question_ response. A question contains an implementation of a _question_ combined with a _completer_, both of which are abstractions that allow the controller to deal only with the general problem of user input.
